...
export default function ViewProfile({ transparent, setTransparent, currUser }) {
    ...
    const fetchData = async() => {
        ...
        let data = await axios.get(
            `http://localhost:3001/user/view/${username}`)
        ...
        // Set friend status
        if (data.data.user.sentFriendRequests != null && 
        ...
        if (data.data.user.incomingFriendRequests != null && 
            ...
            for (let i = 0; i < data.data.user.incomingFriendRequests.length; 
                i++) {    
                if (data.data.user.incomingFriendRequests[i] == 
                    currUser.username) {
                    setFriendStatus("sent")
                    return
                }
        ...
        setFriendStatus("no")
        if (data.data.user.friends != undefined && 
            data.data.user.friends != null) {
 ...
 * @returns 
 */
export function ViewProfileBanner({ currUser, posts, profileData, 
    select, setSelect, acceptFriend, addFriend, friendStatus={friendStatus} }) {
        ...
        <>
            <div className="view-profile-banner">
                ...
                <ul className="view-profile-nav">
                    <li className={`view-profile-posts-button 
                        ${select != "posts" ? "" : "active"}`} 
                        onClick={() => {
                            if (select != "posts") {
                                setSelect("posts")
                    ...
                    <li className={
                        `view-profile-about-button ${select == "posts" ? 
                        "" : "active"}`} onClick={() => {
                            if (select == "posts") {
                                setSelect("about")
                            }    
